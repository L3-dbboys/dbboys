...
public class RemoteUninstallerUtil {
    ...
    private static void initMainDialog(Stage parent) {
        ...
        mainDialog.setTitle("远程卸载向导 - 步骤 1/3");
        ...
        contentStack.getChildren().addAll(step1Pane, step2Pane, step3Pane,backgroupHbox);
    ...
    private static Node createStep1Content(Stage parent) {
        ...
        Label descbefore=new Label("请填写需要远程卸载数据库的服务器信息：");

        Label desc=new Label("说明：");
        Label desc1=new Label("1、远程卸载仅用于Linux或Unix系统远程卸载，不适用于Windows系统。");
        Label desc3=new Label("2、远程卸载会自动卸载之前已存在的GBase 8s数据库安装，并清理所有相关信息。");
        Label desc4=new Label("3、远程卸载向导支持GBase 8s V8.7、GBase 8s V8.8。");
    ...
    private static Node createStep2Content(Stage parent) {
        customInstallStepHbox1=new CustomInstallStepHbox("・关闭数据库进程","kill所有owner为gbasedbt用户进程。");
        customInstallStepHbox2=new CustomInstallStepHbox("・删除安装目录","删除/GBASEDBTTMP/.infxdirs记录的所有目录，删除/GBASEDBTTMP目录，删除/opt/gbase。");
        customInstallStepHbox3=new CustomInstallStepHbox("・删除数据文件","删除所有owner为gbasedbt用户的文件。");
        customInstallStepHbox4=new CustomInstallStepHbox("・删除用户目录","如果存在/etc/gbasedbt，删除该目录。");
        customInstallStepHbox5=new CustomInstallStepHbox("・删除用户及组","删除gbasedbt用户及gbasedbt组。");
        HBox titleHBox=new HBox(new Label("点击【下一步】开始卸载"));
    ...
    private static Node createStep3Content(Stage parent) {
        ...
        StackPane stackPane=new StackPane(new Label("卸载完成！"));
        // 进入步骤2时自动加载信息
        //loadSystemInfo();
    ...
    private static void updateWizardState() {
        mainDialog.setTitle("远程卸载向导 - 步骤 " + currentStep + "/3");
        showCurrentStep();
        updateButtonStates(
    ...
    private static void updateButtonStates(Button previous, Button next, Button finish,Button cancel) {
        ...
        boolean showFinish = currentStep == 3;
