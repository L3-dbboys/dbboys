...
class InstallDownloadTaskWrapper {
    ...
    private final VBox rootPane; // StackPane 的子节点
    ...
    public InstallDownloadTaskWrapper(Object param, File file, boolean autoCloseOnComplete,ResultSetMetaData metaData) {
        this.param=param;
        this.file = file;
        ...
        if(param instanceof String) {
            ...
            HBox hBox1 = new HBox(6, progressBar,progressLabel, buttonBox);
            hBox1.setAlignment(Pos.CENTER_LEFT);
            HBox hBox2 = new HBox(6, nameLabel,   speedLabel);

            rootPane=new VBox(hBox1,hBox2);
            ...
            rootPane = new VBox(6, nameLabel,progressBar,progressLabel, stopButton);
    ...
    private void stackPaneRemoveSelf() {
        ...
        InstallAutoDownLoadUtil.removeDownload(this);
        Platform.runLater(() -> {
...
public class InstallAutoDownLoadUtil {

    public static StackPane stackPane; // 替代 TabPane
    private static List<InstallDownloadTaskWrapper> downloads = new ArrayList<>();
    private static int currentIndex = 0;
    public static CustomUserTextField remotePathField;
    public static CustomUserTextField installFilePathField;
    static {
        stackPane= null;
        // 自动轮播
        Thread switcher = new Thread(() -> {
            try {
                while (true) {
                    ...
                    Platform.runLater(InstallAutoDownLoadUtil::showNext);
                }
    ...
    public static void addDownload(Object url, File file, boolean autoCloseOnComplete, ResultSetMetaData metaData) {
        if(file.exists()){
            ...
            AlterUtil.CustomAlert("下载错误","该文件在桌面已存在，无需重复下载！");
        ...
        if(tempFile.exists()){
            Platform.runLater(() -> {
               AlterUtil.CustomAlert("下载错误","该文件已在下载，已自动填充路径，无需重复下载！");
            });
            return;
        }
        InstallDownloadTaskWrapper wrapper = new InstallDownloadTaskWrapper(url, file, autoCloseOnComplete,metaData);
    ...
    /** 停止所有任务 */
    public void stopAll() {
        downloads.forEach(InstallDownloadTaskWrapper::cancelDownload);
    }
    ...
    public static void removeDownload(InstallDownloadTaskWrapper wrapper) {
