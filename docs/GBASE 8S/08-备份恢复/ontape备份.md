...
### 配置自动备份
#### 配置定时物理备份  
【gbasedbt】用户执行以下命令生成备份脚本：
```shell
cat <<! >/opt/gbase/scripts/backup.sh
#!/bin/bash
