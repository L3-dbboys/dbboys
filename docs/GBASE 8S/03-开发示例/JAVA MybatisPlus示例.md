...
<mapper namespace="com.example.springbootdemo.mapper.UserMapper">
    ...
    <!-- 批量提交，用#{userName}可能会报错主变量给的过多或是不足。 -->
    ...
    <!-- 多条update用#{userName}可能会报错主变量给的过多或是不足。 -->
    <update id="updateUser" parameterType="com.example.springbootdemo.pojo.User">
...
class SpringbootdemoApplicationTests {
    ...
    void testForeachAddUser() { //循环插入，每行一提交
        ...
        System.out.println("foreach insert 1000 users used time:"+(end-start)+"ms");
    ...
    void testBatchAddUserUnionAll() { //批量插入，使用union all
        ...
        Long end=System.currentTimeMillis();
        System.out.println("batch union all insert 1000 users used time:"+(end-start)+"ms");
    ...
    void testAddUser() { //单行插入，批量提交
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if ( i % BATCH == 0) {
                ...
                sqlSession.rollback();
                Long end=System.currentTimeMillis();
                System.out.println("add user commit once insert 1000 users used time:"+(end-start)+"ms");
            }
        }
        sqlSession.commit();
        Long end=System.currentTimeMillis();
    ...
    void testUpdateUser() { //更新
        ...
        Long end=System.currentTimeMillis();
        System.out.println("user updated used time:"+(end-start)+"ms");
