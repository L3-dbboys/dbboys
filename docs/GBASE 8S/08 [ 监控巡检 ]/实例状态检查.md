使用操作系统【gbasedbt】用户ssh或telnet登陆数据库服务器，执行以下命令：
```
onstat -
```
...
```
如出现以下输出，则该数据库实例未启动或已关闭，参考【[数据库异常宕机](/docs/12%20%5B%20应急处理%20%5D/数据库异常宕机)】启动，其中【gbase01】是实例名：
```
...
|reason|【CKPT】被检查点阻塞，参考【[Blocked:CKPT](/docs/12%20%5B%20应急处理%20%5D/Blocked%20CKPT)】处理|
|reason|【LONGTX】长事务阻塞数据库，参考【[Blocked:LONGTX](/docs/12%20%5B%20应急处理%20%5D/Blocked%20LONGTX)】处理|
|reason|【ARCHIVE】备份阻塞数据库，参考【[Blocked:ARCHIVE](/docs/12%20%5B%20应急处理%20%5D/Blocked%20ARCHIVE)】处理|
|reason|【LAST_LOG_RESERVED4BACKUP】日志没及时归档，参考【[Blocked:LAST_LOG_RESERVED4BACKUP](/docs/12%20%5B%20应急处理%20%5D/Blocked%20LAST_LOG_RESERVED4BACKUP)】处理|
|reason|【OVERRIDE_DOWN_SPACE】数据库空间状态OFFLINE，参考【[Blocked:OVERRIDE_DOWN_SPACE](/docs/GBase%208s/12%20%5B%20应急处理%20%5D/Blocked%20OVERRIDE_DOWN_SPACE.md)】处理|
|reason|【HA_CONV_STD】数据库集群切换执行中，等待完成即可，如长时间未完成，及时联系厂商支持|
|other|【Generic system Block: -1 seconds】数据库异常被阻塞，【[重启数据库](/docs/12%20%5B%20应急处理%20%5D/重启数据库)】或联系厂商支持|
