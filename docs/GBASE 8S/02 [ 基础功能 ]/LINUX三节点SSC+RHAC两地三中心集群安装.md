...
## 1、安装前准备
### 1.1、安装前准备
参考【[LINUX两节点SSC共享磁盘集群安装](./LINUX两节点SSC共享磁盘集群安装)】安装两节点SSC共享磁盘集群，在此集群中添加RHAC节点即可。

### 1.3、集群架构图
SSC+RHAC集群为GBase 8s两地三中心最佳实践方案，SSC集群集群切换后可避免数据丢失，RHAC为完全异步集群，保证数据同步的同时，最大限度降低对主节点影响，其架构图如下图：
![ssc+rhac集群架构图.png](/img/ssc+rhac集群架构图.png)
