...
<table>
    ...
    <tr>
        <td>1</td>
        <td>多个重复子查询</td>
        <td>select count(\*) from<br />(<br />select max(c_age) from t_user group by c_birthday<br />union all<br />select max(c_age) from t_user group by c_birthday<br />)</td>
        <td>with t as (select max(c_age) as c_age from t_user group by c_birthday)<br />select count(\*) from<br />(<br />select \* from t<br />union all<br />select \* from t<br />)</td>
      	<td>0.497 sec</td>
      	<td>0.298 sec</td>
      	<td>重复的子查询可使用with as先执行子查询，避免子查询重复执行耗费时间</td>
    </tr>
</table>
