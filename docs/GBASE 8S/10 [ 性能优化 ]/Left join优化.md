...
<table>
    ...
    <tr>
        <td>1</td>
        <td>left join (子查询)</td>
        <td>select count(\*) from <br />(<br />
select t1.c_birthday,t2.username<br />
from t_user t1 <br />
left join<br />
(select c_birthday,max(c_name) as username from t_user group by c_birthday) t2<br />
on t1.c_birthday=t2.c_birthday<br />)</td>
        <td>select count(\*) from<br />
(<br />
select t1.c_birthday,(select max(t2.c_name) from t_user t2 <br />
          where t1.c_birthday=t2.c_birthday) as username<br />
from t_user t1 <br />)</td>
      	<td>0.890 sec</td>
      	<td>0.002 sec</td>
      	<td>left join一个子查询，如果子查询的join字段是唯一的，可以将子查询需要查询的字段写到上一层select中，去掉该子查询的join</td>
    </tr>
    <tr>
        <td>2</td>
        <td>left join on xxx or xxx</td>
        <td>select count(\*) from<br />
(<br />
select t1.c_birthday,t2.c_name<br />
from t_user t1<br />
left join t_user t2<br />
on<br />
(t1.c_name='张三' and t1.c_birthday=t2.c_birthday) <br />
or<br />
(t1.c_name='李四' and t1.c_phone=t2.c_birthday) <br />
)</td>
        <td>select count(\*) from<br />
(<br />
select t1.c_birthday,t2.c_name<br />
...
left join t_user t2<br />
on<br />
(case when t1.c_name='张三' then t1.c_birthday when t1.c_name='李四' then t1.c_phone <br />end) =t2.c_birthday<br />
)</td>
      	<td>--</td>
      	<td>0.994 sec</td>
      	<td>join字段存在不一样的列join时，考虑使用case when改写</td>
    </tr>
</table>
