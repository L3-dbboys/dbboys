...
<table>
    ...
    <tr>
        <td>1</td>
        <td>select first 1 * from t_user where c_birthday='2020-10-01' order by c_name;</td>
        <td>100000</td>
        <td>c_birthday</td>
        <td>c_name</td>
      	<td>无</td>
    ...
    <tr>
        <td>3</td>
        <td>select first 1 * from t_user where c_sex='男' order by c_name;</td>
        <td>100000</td>
        <td>c_sex</td>
        <td>c_name</td>
      	<td>无</td>
...
过滤字段选择度高的情况下，在过滤字段创建索引，反之在order by字段创建索引，视情况考虑在过滤字段和排序字段创建复合索引，排序字段在前
