...
## 1、安装前准备
### 1.1、安装前准备
参考【[LINUX安装前准备](docs/GBase 8s/03 [ 安装配置 ]/LINUX安装前准备.md)】
### 1.2、所需安装组件
RHAC集群只支持多个备节点，自动切换需依赖连接管理器，如不配置自动切换，即需要安装以下一个安装包：
<table>
  <tr>
...
```
### 2.3、主节点onconfig参数调整
修改LOG_INDEX_BUILDS为1，RSS_FLOW_CONTROL为-1，【gbasedbt】用户执行：
```
sed -i "s#^LOG_INDEX_BUILDS.*#LOG_INDEX_BUILDS             1#g" $GBASEDBTDIR/etc/$ONCONFIG
sed -i "s#^RSS_FLOW_CONTROL.*#RSS_FLOW_CONTROL             -1#g" $GBASEDBTDIR/etc/$ONCONFIG
```
...
ha_rhac  onsoctcp        192.168.1.2      9099    g=ha_group
EOF
```
### 2.5、主节点重启数据库
【gbasedbt】用户执行：
...
```
集群有心跳IP配置：如主机心跳IP为192.168.1.1，主节点配置对备节点心跳IP信任即可，【root】用户执行：
```
